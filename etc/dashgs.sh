...
case "$1" in
	start)
		RUNNING=`ps -ef|grep "/usr/bin/node /var/adm/ssoo/dashboard/bin/www"|grep -v grep |wc -l`
		if [ "$RUNNING" -eq "1" ]; 
		then 
		...
		exec /usr/bin/forever start -s --minUptime 10000  --spinSleepTime 1000 /var/adm/ssoo/dashboard/bin/www 2>&1 >/dev/null
		echo "done."
		;;
	stop)
		echo -n "Stopping dashboard..."
		exec /usr/bin/forever stop /var/adm/ssoo/dashboard/bin/www 2>&1 >/dev/null
		echo "done."
		;;
	restart)
		echo -n "Restarting dashboard..."
		exec /usr/bin/forever restart /var/adm/ssoo/dashboard/bin/www 2>&1 >/dev/null
		echo "done."
		;;
	*)
